...
> **用途**: 本项目 ESPRIT 频率估计的完整数学流程 + 实现指导。  
> 
> ⚠️ **已知缺陷 — 请勿直接按此文档编码**
>
> 本文档的 Q 变换块公式（§3 Step 3）和 K₂ 索引公式（§5.3 vs §6.2）经
> 独立 LLM 会话交叉验证发现存在关键错误：
> - **Q 变换块公式**：稀疏块公式生成块对角 T_X（零副对角块），使协方差
>   `R = T_X·T_X^T` 也为块对角。EVD 特征向量只能看到一半信号结构。
>   该公式在数学上无效——Q 变换不会产生块对角结果。根因：该捷径公式
>   错误地省略了 Z_CH 的跨列混合效果。
> - **K₂ 索引公式矛盾**：§5.3 与 §6.2 给出不同的 K₂ 索引方式。§6.2 版本
>   产生 `K₂Es` 的线性相关行（`row(L-3)` = −`row(L-4)`等），使 LS 正规
>   方程求解器产生错误特征值。
>
> **当前代码实际使用的方法**（`src/estimator/esprit.cpp`, v2.1）：
> 放弃本文件的 Q 变换 + K₁/K₂ 管线，改为在 FB 平均实协方差上使用
> **标准 ESPRIT 选择矩阵**（U₁ = E_s.topRows(L-1), U₂ = E_s.bottomRows(L-1)）。
> 保留的优化元素来自本文件的分析：自适应 L（§四）、全实数协方差 + EVD、
> 正规方程 LS（§6.2 LDlT）、跳过 Vandermonde 重建。当前实现可能 OK。
>
> **本文件保留作为分析参考文献**，代码实现请以 `src/estimator/esprit.cpp` 为准。
